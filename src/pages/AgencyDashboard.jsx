  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] text-[#1a1a1c] font-sans p-6 md:p-8 text-left">
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 flex flex-col md:flex-row md:justify-between md:items-end gap-4 mt-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-black mb-2 flex items-center gap-2 md:gap-3 text-purple-600">
              <ShieldCheck size={32} className="md:w-9 md:h-9" /> 팬덤 보이스 모니터링
            </h1>
            <p className="text-gray-500 text-sm font-medium">소속사 전용 관리 화면입니다. 팬들의 의견을 가감 없이 실시간으로 모니터링합니다.</p>
          </div>
          <button onClick={() => setSelectedCategory('전체')} className="px-6 py-2 rounded-2xl text-xs font-bold transition w-full md:w-auto bg-white border border-gray-200 text-gray-500 hover:border-purple-600/30">전체보기</button>
        </header>
        ...
        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] border border-gray-200 overflow-hidden shadow-sm mb-20">
              ...
              <div key={post.id} className="flex flex-col md:flex-row p-6 md:p-8 hover:bg-gray-50 transition-colors group gap-4 md:gap-8 items-start md:items-center border-b border-gray-200 last:border-b-0">
                ...
                <div className="w-full md:flex-1">
                  <Link to={`/post/${post.id}`} className="block">
                    ...
                    {/* 날것의 피드백을 전부 읽을 수 있도록 줄임표(line-clamp) 제거 */}
